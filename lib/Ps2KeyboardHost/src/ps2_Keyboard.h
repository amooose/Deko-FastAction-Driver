...
namespace ps2 {
    ...
    public:
		...
		bool sendBytes(const byte *data, int numBytes=2, unsigned int del=0) {
            this->transmitting=true;
            for (int i = 0; i < numBytes; ++i) {
                if (!this->sendData(data[i])) {
                    Serial.println("BAD IN DATA");
                    if(!this->echo(1000)){
        ...
        KeyboardOutput sendGetResponse(const byte *data,  uint16_t timeoutInMillis, int numBytes= 2) {
            this->sendBytes(data, numBytes);
             return expectResponse(timeoutInMillis);
        }
        bool expectAck() {
        ...
        */
         ...
         *  \returns Returns true if the keyboard responded appropriately and false otherwise.
         */
        bool echo(uint16_t timeoutInMillis = 1000) {
            this->sendByte((byte)ps2CommandCode::echo);
            return this->expectResponse(KeyboardOutput::echo,timeoutInMillis);
        }
